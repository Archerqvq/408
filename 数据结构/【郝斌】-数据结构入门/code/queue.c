...
int main(void)
{
    ...
    en_queue(&Q, 5);
    en_queue(&Q, 6);
    en_queue(&Q, 7); // 队列已满，不再入队
    en_queue(&Q, 8); // 队列已满，不再入队
    out_queue(&Q, &val);
    out_queue(&Q, &val);
    out_queue(&Q, &val);
    out_queue(&Q, &val);
    out_queue(&Q, &val);
    out_queue(&Q, &val);
    out_queue(&Q, &val); // 队列已空，不再出队
    en_queue(&Q, 6); // rear变为0，重新入队
    en_queue(&Q, 7);
    en_queue(&Q, 8);
    out_queue(&Q, &val); // front变为0，重新出队
    out_queue(&Q, &val);
...
{
    if (((pQ->rear + 1) % 6) == (pQ->front)) // 通过存放n-1个队列成员的方式来区分满队列和空队列的区别
    {
        return true;
    }
    else
